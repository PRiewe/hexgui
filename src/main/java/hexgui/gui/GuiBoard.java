...
public final class GuiBoard extends JPanel implements Printable {
  ...
  /**
   * Sets the type of board drawer to use. If <code>name</code> is not one of the known values,
   * "Diamond" is used.
   *
  ...
  /**
   * Marks the given point to show which move was played last, or clears the mark if <code>point
   ...
   *
   * <p>NOTE: THIS IS BROKEN SINCE HexPoint was changed in wolve, r182. USE BASE 64 INSTEAD!
   *
   * <p>FIXME: switch carriers to be printed as a list of HexPoints instead of as hex strings?
   */
    ...
    public void paintComponent(Graphics graphics) {
      ...
      if (flip) {
        ...
        for (int i = 0; i < m_arrows.size(); i++) {
          ...
          arrows.add(
              new Pair<HexPoint, HexPoint>(HexPoint.get(p1.y, p1.x), HexPoint.get(p2.y, p2.x)));
        }
      }
