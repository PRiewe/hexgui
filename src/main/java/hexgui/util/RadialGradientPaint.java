...
public class RadialGradientPaint implements Paint {
  public RadialGradientPaint(
      Point2D point, Color pointColor, Point2D radius, Color backgroundColor) {
    ...
    if ((alphaPoint & alphaBackground) == 0xff) m_transparency = OPAQUE;
    else m_transparency = TRANSLUCENT;
  }
  ...
  public PaintContext createContext(
    ...
    if (m_cachedContext != null
        ...
        && transformedRadius.equals(m_transformedRadius)) return m_cachedContext;
    m_transformedPoint = (Point2D) transformedPoint.clone();
    m_transformedRadius = (Point2D) transformedRadius.clone();
    m_cachedContext =
        new RadialGradientContext(
            transformedPoint, m_pointColor,
